...
<template>
  <el-card class="box-card" style="background: #f7f7f7">
    ...
    <el-row>
      ...
      <el-col :span="24">
        <el-button type="primary" icon="el-icon-plus" id="pjkjAdd" size="mini" @click="addDysq()">预开票据</el-button>
      </el-col>
      <el-col :span="24">
        <el-table ref="multipleTable" :data="tableData" :header-cell-style="{background:'#e6e6e6'}"
          ...
          <el-table-column prop="pjmc" label="票据名称" :show-overflow-tooltip="true" min-width="200px">
            <template slot-scope="scope">
              <span>{{ scope.row.pjmc }}</span>
            </template>
          </el-table-column>
          <el-table-column prop="dwmc" label="单位名称" :show-overflow-tooltip="true" min-width="200px"></el-table-column>
          <el-table-column prop="cftk" label="处罚条款" :show-overflow-tooltip="true"></el-table-column>
...
<!--          <p>{{ylobj.ay}}</p>-->
        ...
        <div style="line-height:26px">
          <div style="position: relative; top: 146px; left: 75px" v-for="item in yllist">
  ...
  export default {
    ...
    methods: {
      ...
      findTable() {
        ...
        this.$api.pjkj.queryTable(param).then((res) => {
