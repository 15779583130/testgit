              ...
              <div class="form-panel">
                <el-form ref="dataForm" :model="dataForm" size="mini" :rules="dataFormRules">
                  <el-row class="form-line">
                    ...
                    <el-col :span="8">
                      <el-form-item label="被没收人" prop="bmsr">
                        <el-input v-model="dataForm.bmsr" size="mini"  >
                        </el-input>
                      </el-form-item>
                    </el-col>
                    <el-col :span="8">
  ...
  export default {
    ...
    methods: {
      init() {
        ...
        var id = window.location.href.split('pjkjkp/')[1];
        this.id = id
        this.queryDataForm();
      },
      deleteRightMx(id){
        ...
        this.$api.pjkj.delClmx(param).then((res) => {
          if (res.success == "true") {
            let param1 = {
              id: this.id,
              ajxx_id:this.dataForm.ajxx_id
            }
            this.$api.pjkj.querywzTable2(param1).then((res) => {
            ...
            this.$api.pjkj.querywzTable3(param).then((res) => {
              // this.pageinfo.total = Number(res.body.default.count);
      ...
      createForm(){
        ...
        this.$api.pjkj.saveOrUpdateClmx(param).then((res) => {
          if (res.success == "true") {
            let param1 = {
              ajxx_id: this.id
            }
      ...
      queryDataForm(){
        ...
        this.$api.pjkj.queryDataForm(param).then((res) => {
          this.dataForm = res.body.default;
          this.$api.pjkj.querywzTable2(param).then((res) => {
            // this.pageinfo.total = Number(res.body.default.count);
            this.tableData = res.body.default.dataset||[]
            this.checkList  = res.body.default.dataset||[]
          })
          ...
          this.$api.pjkj.querywzTable3(param1).then((res) => {
            // this.pageinfo.total = Number(res.body.default.count);
            this.rightList = res.body.default.dataset||[]
          })
      ...
      deletePj(){
        ...
        this.$confirm('是否确定删除该票据记录?', '提示', {
          ...
          this.$api.pjkj.deletePj(param).then((res) => {
            if (res.success == "true") {
              this.$message({message: res.message, type: 'success'})
              this.close();
            } else {
        ...
        this.$refs.dataForm.validate((valid) => {
          if (valid) {
            this.$confirm('是否确认开具发票信息？', '提示', {
              ...
              this.$api.pjkj.checkPj(param).then((res) => {
                if (res.success == "true") {
                    // this.$message({message: res.message, type: 'success'})
                  this.$confirm(res.message, '提示', {
                    ...
                    this.$api.pjkj.submitPjzb(params).then((res) => {
                      if (res.success == "true") {
                        this.$message({message: res.message, type: 'success'})
                        this.close();
                      } else {
