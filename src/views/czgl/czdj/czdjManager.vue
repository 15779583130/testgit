...
<template>
  <el-card class="box-card" style="background: #f7f7f7">
    ...
    <el-row>
      ...
      <el-col :span="24">
        <el-button type="primary" icon="el-icon-plus" v-access id="czdjAdd" size="mini" @click="addDysq()">处置申请</el-button>
  ...
  export default {
    ...
    methods: {
      ...
      viewClick(row) {
        ...
        this.$api.czdj.getCzdjAndAjxxById(params).then((res) => {
          this.$nextTick(() => {
            sessionStorage.removeItem('czsqck')
            sessionStorage.setItem('czsqck',  JSON.stringify(res.body.default))
            this.$router.push({
              name: 'czdjView',
      ...
      findTable() {
        ...
        this.$api.czdj.queryTable(parms).then((res) => {
      ...
      findCzlcTable() {
        ...
        this.czlcTotal = 0
        this.czlcTableData = []
        this.$api.czdj.queryCzlc(param).then((res) => {
      ...
      delDysq(row) {
        this.$confirm('确定删除该处置申请？', '提示', {
          ...
          this.$api.czdj.del(params).then((res) => {
            if (res.success === "true") {
      ...
      submitClick(row){
        this.$confirm('确定是否提交该处置申请？', '提示', {
          ...
          this.$api.czdj.submitCzsqForManager(params).then((res) => {
            if (res.success === "true") {
              this.$message({message: res.message, type: 'success'})
            } else {
              this.$message({message: res.message, type: 'error'})
      ...
      chooseAjxx(row) {
        ...
        this.$api.czdj.getCzdjAndAjxxById(params).then((res) => {
          this.$nextTick(() => {
            ...
            sessionStorage.removeItem('czdjbj')
            sessionStorage.setItem('czdjbj',  JSON.stringify(res.body.default))
            this.$router.push({
              name: 'czdjEdit',
              params: res.body.default
            })
