      ...
      <el-col :span="24">
        <el-table ref="multipleTable" :data="tableData" tooltip-effect="dark" style="width: 100%" size="mini" :header-cell-style="{background:'#e6e6e6'}"
          ...
          <el-table-column label="操作" fixed="right" align="center" width="120px">
            <template slot-scope="scope">
              <i v-if="scope.row.pdxx_jjzt =='0' && scope.row.state!='0' && scope.row.state!='1' && scope.row.state!='7'" class="el-icon-edit"
...
<script>
  ...
  export default {
    ...
    methods: {
      init() {
        this.formInline = JSON.parse(sessionStorage.getItem('czsqck'))
      ...
      configWlpd () {
        this.$confirm('是否确认完成？', '提示', {
          ...
          let that = this
          this.$api.czdj.wlpdHandPver(params).then((res) => {
            if (res.success === 'true') {
              ...
              that.$api.czdj.exportWlpdWord(param).then((res) => {
                if (res.success === 'true') {
      ...
      findTable() {
        ...
        this.$api.czdj.queryYwcbForBxqd(param).then((res) => {
          ...
          this.tableData.forEach(function (c) {
            ...
            that.$api.czdj.queryWzmx(param2).then((res) => {
      ...
      pdjj (row, type) {
        ...
        this.$api.czdj.getWlpdInfo(param).then((res) => {
          this.$nextTick(() => {
            if (Object.keys(res.body).length != 0) {
              ...
              this.pdjjDialog = false
               this.$message({message: '后台数据异常，未查询到派单信息，请联系管理员！', type: 'error'})
            }

          })
        })
        this.$api.czdj.getWlpdTxryxx(param).then((res) => {
          this.$nextTick(() => {
        ...
        this.$api.czdj.queryWlpdJjWzxx(param).then((res) => {
          this.$nextTick(() => {
