...
<template>
  <card>
    <Form  :model="formData"  ref="formData" :label-width="120" :rules="ruleValidate">
      <Row>
        <Col span="10" offset="" >
          <h4 style="display:inline">编辑错误码</h4>
...
export default {
  ...
  methods: {
    ...
    getOrganizationDetail () {
      let self = this
      this.$axios.get('channelRespcode/' + this.formData.id)
    ...
    handleSubmit () {
      ...
      this.$refs['formData'].validate((valid) => {
        if (valid) {
          this.$axios.put('/channelRespcode', this.formData).then(function (res) {
            that.$Message.success('编辑错误代码成功')
            that.goBack()
          })
        } else {
