...
<template>
  <el-card class="box-card" style="background: #f7f7f7">
                ...
                <el-row class="form-line">
                  <el-col :span="19" style="display: flex;">
                    ...
                    <el-form-item label="状态" prop="zt">
                      <el-select v-model="filters.zt" placeholder="请选择">
                        <el-option value="0" label="草稿"></el-option>
                        <el-option value="1" label="正文"></el-option>
                      </el-select>
                    </el-form-item>
        ...
        <el-row>
          <el-col :span="24">
            <el-form-item label="标题" prop="bt" style="width: 100%">
              <el-input v-model="dataForm.bt" placeholder="请输入标题" :readonly="!isEdit"></el-input>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <template v-if="!dataForm.fbsj">
            <el-col :span="12">
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="通知类型" prop="tzlx" style="width: 100%">
              <el-select v-model="dataForm.tzlx" style="width:100%" :disabled="!isEdit">
                <el-option value="0" label="通知"></el-option>
                <el-option value="1" label="通告"></el-option>
                <el-option value="2" label="通报"></el-option>
                <el-option value="3" label="公告"></el-option>
            ...
            <el-form-item label="公告时间">
              <el-col :span="10" style="display: flex;">
                <el-date-picker
                  ...
                  type="date"
                  placeholder="请输入日期起" size="mini" format="yyyy-MM-dd" style="width: 160px"
                  value-format="yyyy-MM-dd" :readonly="!isEdit">
                </el-date-picker>
              </el-col>
              <el-col :span="2" style="display: flex;">
                <i class="text">-</i>
              ...
              <el-col :span="10" style="display: flex;">
                <el-date-picker
                  ...
                  type="date"
                  placeholder="请输入日期止" size="mini" format="yyyy-MM-dd"
                  value-format="yyyy-MM-dd" style="width: 160px" :readonly="!isEdit">
                </el-date-picker>
              </el-col>
            </el-form-item>
          </el-col>
        ...
        <el-row>
          <el-col :span="24">
            <el-form-item label="接收部门" prop="viewdeptnames" style="width: 100%" v-if="isEdit">
              <el-input v-model="dataForm.viewdeptnames" placeholder="请选择部门" :readonly="true" @focus="chooseDept()"></el-input>
            </el-form-item>
            <el-form-item label="接收部门" prop="viewdeptnames" style="width: 100%" v-if="!isEdit">
        ...
        <el-row style="margin-top: 15px">
          <el-col :span="24">
              <el-form-item label="上传文件">
                <el-button type="success" size="small" @click="dialogUpLoadClick" plain v-if="isEdit">上传文件</el-button>
                <el-button type="success" size="small" @click="dialogUpLoadClick" plain v-if="!isEdit">查看文件</el-button>
              </el-form-item>
          </el-col>
        </el-row>
      </el-form>
      <span slot="footer" class="dialog-footer" style="margin-top: 5px">
            <el-button type="primary" @click="saveOrupdate(0)" size="mini" >草稿</el-button>
            <el-button @click="saveOrupdate(1)" size="mini" >正文</el-button>
    ...
    <el-button @click="babmDialog = false" size="mini" v-if="btnshow">取 消</el-button>
    <el-button type="primary" @click="getChooseDept" size="mini" v-if="btnshow">确 定</el-button>
    <el-button @click="babmDialog = false" size="mini" v-if="!btnshow">关闭</el-button>
  </span>
    </el-dialog>
      ...
      return {
        ...
        dataFormRules: {
          bt: [
            ...
            {min: 0, max: 50, message: '长度不得超过50字符'}
          ],
          tzlx: [
    ...
    methods: {
      ...
      saveOrupdate(lx){
        this.$refs.dataForm.validate((valid)=>{
          if(valid){
            // this.dataForm.ggnr = this.escapeHTML(this.content)
            // this.dataForm.ggnr = window.btoa(encodeURI(this.content))
            this.dataForm.zt  = lx;
            var param = this.dataForm;
            param.base64_content =window.btoa(encodeURI(this.content))
            this.$api.ggmanage.saveOrupdate(param).then((res) => {
              if (res.success == "true") {
      ...
      chooseDept() {
        ...
        var param = {};
        let that = this;
        this.$api.ggmanage.deptTree(param).then((res) => {
          if (res.success == "true") {
            this.treeData = res.data
            this.babmDialog = true;
            this.$refs.tree1.setCheckedKeys([]);
            this.$refs.tree1.setCheckedKeys(that.dataForm.viewdeptids.split(","));
          } else {
            this.$message({message: res.message, type: 'error'})
          }
        })
      ...
      findTable() {
        ...
        this.$api.ggmanage.queryTable(param).then((res) => {
      ...
      edit(row){
        ...
        this.isEdit = true
        if(row.zt==='1'){
          this.$message({message:'草稿状态允许修改,请先撤回公告', type: 'warning'})
          return false
        }
        var params = {
          id: row.id
      ...
      reback(row){
        ...
        this.$api.ggmanage.reback(param).then((res) => {
          if (res.success == "true") {
            this.findTable()
            this.$message({message: res.message, type: 'success'})
          } else {
      ...
      del(row){
        ...
        this.$confirm('确定要删除当前的记录吗?', '提示', {
          ...
          this.$api.ggmanage.getGgqx(param).then((res) => {
            if (res.success == "true") {
              this.$api.ggmanage.del(param).then((res) => {
                if (res.success == "true") {
                  this.findTable()
                  this.$message({message: res.message, type: 'success'})
                }else {
