...
<template>
  <el-card class="box-card" style="background: #f7f7f7">
    ...
    <el-row>
            ...
            <div class="form-panel">
              <el-form ref="form" :model="formInline" size="mini" :inline="true">
                ...
                <el-row class="form-line" v-show="more">
                  ...
                  <el-form-item label="入库流水号">
                    <el-input v-model="formInline.rklsh" placeholder="请输入入库流水号" size="mini" clearable
                              @keyup.enter.native="findTable">
                    </el-input>
                  </el-form-item>
      ...
      <el-col :span="24">
        <el-button type="primary" icon="el-icon-plus" v-access id="rksqAdd" size="mini" @click="addDysq()">新增</el-button>
      ...
      <el-col :span="24">
        <el-table ref="multipleTable" :data="tableData" :header-cell-style="{background:'#e6e6e6'}"
          ...
          <el-table-column prop="ajmc" label="案件名称" :show-overflow-tooltip="true" min-width="150px">
            <template slot-scope="scope">
              <span style="color:#5287d8" @click="handlerByName(scope.row)">{{ scope.row.ajmc }}</span>
            </template>
          </el-table-column>
          <el-table-column prop="lsh" label="入库流水号" width="120px" :show-overflow-tooltip="true"></el-table-column>
          <el-table-column prop="sqrxm" label="申请人" width="100px" :show-overflow-tooltip="true"></el-table-column>
          <el-table-column prop="sqbmmc" label="申请部门" min-width="150px" :show-overflow-tooltip="true"></el-table-column>
          <el-table-column prop="sqsj" label="申请时间" min-width="120px" :formatter="dateFormat"
                           :show-overflow-tooltip="true"></el-table-column>
          <el-table-column prop="ddsl" label="订单数量" min-width="80px" :show-overflow-tooltip="true"></el-table-column>
          <el-table-column prop="zjzt" label="状态" width="60px" :formatter="formatZjzt"
                           :show-overflow-tooltip="true"></el-table-column>
          <el-table-column
          label="操作" width="120px" fixed="right" align="center">
            <template slot-scope="scope">
              <i v-if="scope.row.zjzt=='0' " v-access id="rksqEdit" class="el-icon-edit" style="font-size: 20px;" title="编辑"
                 @click="editClick(scope.row)"></i>
              <i v-if="scope.row.zjzt=='1'||scope.row.zjzt=='2' || scope.row.zjzt=='9'" class="el-icon-view"
                 style="font-size: 20px;" @click="viewClick(scope.row)" title="查看"></i>
              <i v-if="(scope.row.zjzt=='0' ) || (scope.row.zjzt=='1' && scope.row.isexistwl!='1')" class="el-icon-delete" style="font-size: 20px;"
                 @click="delDysq(scope.row)"  v-access id="rksqDel" title="删除"></i>
            </template>
          </el-table-column>
        </el-table>
        <div class="page">
  ...
  export default {
    ...
    methods: {
      ...
      viewClick(row) {
        ...
        this.$api.rksq.lookRksqxxYwzb(params).then((res) => {
          this.$nextTick(() => {
            if (Object.keys(res.body).length != 0) {
              sessionStorage.removeItem('ckrksqxx')
              sessionStorage.setItem('ckrksqxx', JSON.stringify(res.body.default))
              this.$router.push({
      ...
      findTable() {
        ...
        this.$api.rksq.queryTable(param).then((res) => {
