...
export const upDataPic = (params) => {
  if (global.reqService === 'local') {
    ...
    return new Promise((resolve, reject) => {
      ...
      http.post('/service/file/', 'uploadFilesForBsea64', params).then((data) => {
        var findPageData = {
          pagenum: pagenum,
          pagesize: pagesize,
          totalSize: Number(data.getBody().getNode().getNodeItem('count')),
          content: data.getHeader()
        }
        resolve(findPageData);
      });
    })
...
export const getPicWuzheng = (params) => {
  if (global.reqService === 'local') {
    var data = EnvelopUtil.convertXmlFileToXml('home/data/homeUser.xml', 'none')
    let pagenum = 1
    let pagesize = 10
    ...
    return new Promise((resolve, reject) => {
      ...
      http.post('/service/file', 'getFilesForUrl', params).then((data) => {
        ...
        var findPageData = {
          ...
          trueOrFalse: data.getHeader().name
        }
        resolve(findPageData);
      });
    })
...
export const delPicWuzheng = (params) => {
  if (global.reqService === 'local') {
    var data = EnvelopUtil.convertXmlFileToXml('home/data/homeUser.xml', 'none')
    let pagenum = 1
    let pagesize = 10
    ...
    return new Promise((resolve, reject) => {
      ...
      http.post('/service/file', 'delFilesForBsea64', params).then((data) => {
        var findPageData = {
          pagenum: pagenum,
          pagesize: pagesize,
          totalSize: Number(data.getBody().getNode().getNodeItem('count')),
          content: data.getHeader().name
        }
        resolve(findPageData);
      });
    })
