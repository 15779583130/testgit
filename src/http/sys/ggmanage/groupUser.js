...
export const queryTable =(params)=>{
  ...
  if(global.reqService=="local" && temp){
    ...
    return new Promise((resolve,reject)=>{
      http.post('service/xtgl/group', 'queryUser', params, true, '').then((data)=>{
        resolve(data.getBody().getValueofBody());
      });
    })
  }
 ...
 * @modify  1.00  2019/12/26   dongshanshan   无     新建
 */
export const querySelectUser =(params)=>{
  var temp = false;
  if(global.reqService=="local" && temp){
    return new Promise((resolve,reject)=>{
      var data = EnvelopUtil.convertXmlFileToXml("ckgl/bmckgl/wzcw/data/queryTable.xml", 'none')
    ...
    return new Promise((resolve,reject)=>{
      http.post('service/xtgl/group', 'querySelectUser', params, true, '').then((data)=>{
        resolve(data.getBody().getValueofBody());
      });
    })
  }
...
export const saveOrDel =(params)=>{
  ...
  if(global.reqService=="local" && temp){
    ...
    return new Promise((resolve,reject)=>{
      http.post('service/xtgl/group', 'saveOrDelGroupUser', params, true, '').then((data)=>{
        var obj ={
